...
Expected on a clean Cuttlefish build (4 models + a voice WAV in `oir-vendor-models`):
...
Summary: 5 runnable, 3 unbacked, 4 model-missing
```

Launch **OirDemo** from the app drawer → tap **Fire All**. The 5 demo tiles (`text.complete`, `text.translate`, `text.embed`, `audio.transcribe`, `vision.detect`) attempt to fire — the first 4 stream to completion; `vision.detect` shows a clean MODEL_MISSING error until an OEM bakes the detector.
